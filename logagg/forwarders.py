...
class MongoDBForwarder(BaseForwarder):
    ...
    def __init__(self, host, port, user, password, db, collection, log=DUMMY):
        self.host = host
        self.port = port
        self.user = user
    ...
    def _ensure_connection(self):
        ...
        url = "mongodb://%s:%s@%s:%s" % (self.user, self.passwd, self.host, self.port)
        client = MongoClient(
        ...
        self.log.info(
            "mongodb_collection_created", collection=self.collection, db=self.db_name
        )
    ...
    def handle_logs(self, msgs):
        ...
        try:
            ...
            self.log.info(
                "logs_inserted_into_mongodb", num_records=len(msgs), type="metric"
            )
        except pymongo.errors.AutoReconnect(message="connection_to_mongodb_failed"):
            self._ensure_connection()
        except pymongo.errors.BulkWriteError as bwe:
            self.log.info(
                "logs_inserted_into_mongodb",
...
class InfluxDBForwarder(BaseForwarder):
    ...
    def _ensure_connection(self):
        ...
        self.influxdb_client = InfluxDBClient(
            self.host, self.port, self.user, self.passwd, self.db_name
        )
    ...
    def handle_logs(self, msgs):
        ...
        try:
            ...
            self.log.info(
                "metrics_inserted_into_influxdb",
